...
- 内置 Tavily Web Search，并支持动态加载 Skill 和 MCP 工具。
...
系统设置中的 Web Search 可配置 Tavily API 地址、API Key、超时、结果上限和独立网络代理。API Key 与代理密码使用 AES-256-GCM 加密保存，只对外返回是否已配置；留空保存会保留现有密钥，也可显式清除。搜索查询会发送给 Tavily，返回内容按不可信外部证据交给模型。审计仅记录查询 SHA256、耗时、结果数和代理使用状态，不记录查询正文或凭据。
