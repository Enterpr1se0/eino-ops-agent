...
- Eino `ChatModelAgent` + 24 个强类型 Tools，复杂任务会创建持久化步骤计划并严格逐项推进，支持多套 OpenAI-compatible 模型配置、连通测试与运行时热切换。
...
- SQLite 审计、结构化服务端日志、可取消长任务、历史检索、持久化多会话对话、SSE 流式响应和 React 控制台。
...
服务默认监听 `0.0.0.0:8080`。本机访问 `http://127.0.0.1:8080`，局域网测试使用 `http://服务器IP:8080`。Web 的 **Configuration** 配置中心统一提供模型、SSH 主机和系统设置三个页签；模型页可以添加 OpenAI、DeepSeek、Ollama 或其他 OpenAI-compatible 端点，保存多套配置并点击切换。没有配置模型时服务仍可启动，主机、审计、策略、审批和 CLI 可用，聊天接口返回明确的 `503`。
...
- `ssh_task_start` / `ssh_task_status` / `ssh_task_tail` / `ssh_task_cancel`
- `ssh_file_read` / `ssh_file_list` / `ssh_file_stat` / `ssh_file_write` / `ssh_file_apply_patch`
- `ssh_file_upload` / `ssh_file_download` / `ops_artifact_list`
...
- HTTP 默认监听 `0.0.0.0:8080` 便于局域网测试。当前仍是单用户模式，没有登录认证；不要直接暴露到公网，测试时应通过防火墙限制来源地址。
...
# 大文件先由人类放入受控 artifact 区，再由 Agent 申请 SFTP 传输
./bin/ops-agent artifact add app.tar.gz ./dist/app.tar.gz
./bin/ops-agent artifact upload --host HOST_ID --name app.tar.gz \
  --remote /tmp/app.tar.gz --reason deployment --rollback "rm /tmp/app.tar.gz"
```

## 当前边界

首版是单用户本地控制面，不包含多租户 RBAC、Vault/SSH CA、远程 MCP OAuth 或 Kubernetes 原生 API。内存中的长任务状态不会在进程重启后恢复，但已完成和正在审批的运行记录保留在 SQLite。
