...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><TerminalSquare size={23}/></div><div><strong>OpsPilot</strong><span>{t('shell.brandSubtitle')}</span></div></div>
      <nav>
      ...
      <div className="sidebar-foot">
        ...
        <div className="build">v0.1.0 · {t('shell.localMode')}</div>
      </div>
    </aside>
    <main>
...
function LoginPage({onAuthenticated}:{onAuthenticated:()=>void}){
	...
	const submit=async(event:FormEvent)=>{event.preventDefault();setBusy(true);setError('');try{await api.login(password);setPassword('');onAuthenticated()}catch(err){setError(errorText(err))}finally{setBusy(false)}}
		return <div className="auth-screen"><LanguageSwitch/><section className="login-card"><div className="login-mark"><TerminalSquare size={29}/></div><span>{t('auth.subtitle')}</span><h1>{t('auth.title')}</h1><form onSubmit={submit}><label><span>{t('auth.password')}</span><div className="login-input"><LockKeyhole size={17}/><input type="password" autoComplete="current-password" value={password} onChange={event=>setPassword(event.target.value)} autoFocus required/></div></label>{error&&<div className="login-error"><ShieldAlert size={15}/>{error}</div>}<button className="primary" disabled={busy||password.length===0}>{busy?<LoaderCircle className="spin" size={17}/>:<ShieldCheck size={17}/>}<span>{busy?t('auth.authenticating'):t('auth.enter')}</span></button></form><small>{t('auth.initialPassword')} <code>OPS_AGENT_ADMIN_PASSWORD</code></small></section></div>
}
...
function ExtensionsPage({skills,mcpServers,toolCatalog,refresh}:{skills:ManagedSkill[];mcpServers:MCPServer[];toolCatalog:LLMToolCatalog|null;refresh:()=>Promise<void>}){
	const {t}=useTranslation()
		...
		<div className="configuration-content" role="tabpanel">
			{section==='overview'&&<div className="extension-overview"><button className="panel" onClick={()=>setSection('skills')}><div><BookOpen size={21}/></div><span><small>{t('extensions.knowledge')}</small><h3>Skills</h3><p>{t('extensions.skillsDescription')}</p></span><strong>{enabledSkills}<small>{t('extensions.enabledUnit')}</small></strong><ChevronRight size={16}/></button><button className="panel" onClick={()=>setSection('mcp')}><div><Zap size={21}/></div><span><small>{t('extensions.integrations')}</small><h3>{t('extensions.tabs.mcp')}</h3><p>{t('extensions.mcpDescription')}</p></span><strong>{readyMCP}<small>{t('extensions.readyUnit')}</small></strong><ChevronRight size={16}/></button><button className="panel" onClick={()=>setSection('tools')}><div><FunctionSquare size={21}/></div><span><small>{t('extensions.snapshot')}</small><h3>{t('extensions.loadedFunctions')}</h3><p>{t('extensions.toolsDescription')}</p></span><strong>{toolCatalog?.count??0}<small>{t('extensions.functionsUnit')}</small></strong><ChevronRight size={16}/></button></div>}
...
function MCPServersPage({servers,refresh}:{servers:MCPServer[];refresh:()=>Promise<void>}){
	...
	return <div className="mcp-page page-stack">
			<div className="page-actions"><div><p>{t('mcp.title')}</p></div><button className="primary" onClick={openCreate}><Plus size={15}/>{t('mcp.add')}</button></div>
		...
		{form&&<form className="mcp-form panel" onSubmit={save}><header><div><Zap size={19}/><span><small>{form.id?t('mcp.editEyebrow'):t('mcp.newEyebrow')}</small><h3>{form.id?form.name||t('mcp.server'):t('mcp.connect')}</h3></span></div><button type="button" onClick={()=>setForm(null)} title={t('common.close')}><X size={15}/></button></header><div className="mcp-form-grid"><label><span>{t('mcp.displayName')}</span><input value={form.name} onChange={event=>setForm({...form,name:event.target.value})} placeholder={t('mcp.namePlaceholder')} required/></label><label><span>{t('mcp.transport')}</span><select value={form.transport} onChange={event=>setForm({...form,transport:event.target.value as MCPTransport})}><option value="stdio">{t('mcp.localProcess')}</option><option value="streamable_http">Streamable HTTP</option></select></label>{form.transport==='stdio'?<><label><span>{t('mcp.command')}</span><input value={form.command} onChange={event=>setForm({...form,command:event.target.value})} placeholder="npx" required/></label><label><span>{t('mcp.cwd')}</span><input value={form.cwd} onChange={event=>setForm({...form,cwd:event.target.value})} placeholder="/absolute/path"/></label><label className="mcp-wide"><span>{t('mcp.args')}</span><textarea value={form.argsText} onChange={event=>setForm({...form,argsText:event.target.value})} placeholder={'-y\n@modelcontextprotocol/server-filesystem\n/srv/projects'}/></label></>:<label className="mcp-wide"><span>{t('mcp.endpoint')}</span><input value={form.url} onChange={event=>setForm({...form,url:event.target.value})} placeholder="https://mcp.example.com/mcp" required/></label>}<label className="mcp-wide"><span>{t('mcp.env')}</span><textarea value={form.envText} onChange={event=>setForm({...form,envText:event.target.value,clearEnv:false})} placeholder={form.id?t('mcp.preserve'):'GITHUB_TOKEN=...'}/>{form.id&&<small>{t('mcp.preserve')} <label><input type="checkbox" checked={form.clearEnv} onChange={event=>setForm({...form,clearEnv:event.target.checked,envText:event.target.checked?'':form.envText})}/> {t('mcp.clearEnv')}</label></small>}</label><label className="mcp-wide"><span>{t('mcp.headers')}</span><textarea value={form.headersText} onChange={event=>setForm({...form,headersText:event.target.value,clearHeaders:false})} placeholder={form.id?t('mcp.preserve'):'Authorization: Bearer ...'}/>{form.id&&<small>{t('mcp.preserve')} <label><input type="checkbox" checked={form.clearHeaders} onChange={event=>setForm({...form,clearHeaders:event.target.checked,headersText:event.target.checked?'':form.headersText})}/> {t('mcp.clearHeaders')}</label></small>}</label></div><footer><label className="mcp-enable-on-save"><input type="checkbox" checked={form.enabled} onChange={event=>setForm({...form,enabled:event.target.checked})}/><i/><span><b>{t('mcp.enableAfterSave')}</b><small>{t('mcp.enableAfterSaveText')}</small></span></label><button type="button" onClick={()=>setForm(null)}>{t('common.cancel')}</button><button className="primary" disabled={busy==='save'}>{busy==='save'?<LoaderCircle className="spin" size={14}/>:<Save size={14}/>} {busy==='save'?t('common.saving'):t('mcp.saveServer')}</button></footer></form>}
		...
		{!servers.length&&<Empty icon={<Zap/>} title={t('mcp.emptyTitle')} text={t('mcp.emptyText')}/>}
	</div>
}
...
function SystemSettingsPage({settings,providers,capabilities,modelStatus,refresh}:{settings:SystemSettings|null;providers:ModelProvider[];capabilities:ToolCapabilities;modelStatus?:Health['model'];refresh:()=>Promise<void>}) {
  ...
  return <div className="system-settings page-stack">
		    <div className="page-actions"><div><p>{t('settings.title')}</p></div></div>
    {notice&&<div className="notice">{notice}<button onClick={()=>setNotice('')}><X size={14}/></button></div>}
...
const defaultWebSearchInput:WebSearchSettingsInput={enabled:false,base_url:'https://api.tavily.com',api_key:'',proxy_url:'',proxy_username:'',proxy_password:'',timeout_seconds:20,max_results:10,extract_max_content_kib:32,extract_max_total_kib:128}
...
function WebSearchSettingsPanel({refresh}:{refresh:()=>Promise<void>}){
	...
	const applyStored=(value:WebSearchSettings)=>{setStored(value);setInput({enabled:value.enabled,base_url:value.base_url,api_key:'',proxy_url:value.proxy_url||'',proxy_username:value.proxy_username||'',proxy_password:'',timeout_seconds:value.timeout_seconds,max_results:value.max_results,extract_max_content_kib:value.extract_max_content_kib,extract_max_total_kib:value.extract_max_total_kib});setDirty(false)}
...
function AdminPasswordPanel(){
	...
	const [current,setCurrent]=useState(''),[replacement,setReplacement]=useState(''),[confirmation,setConfirmation]=useState(''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false)
		...
		return <form className="admin-password-panel panel" onSubmit={submit}><div><LockKeyhole size={19}/><span><b>{t('password.title')}</b><small>{t('password.subtitle')} <code>admin reset-password</code></small></span></div><section><label><span>{t('password.current')}</span><input type="password" autoComplete="current-password" value={current} onChange={event=>setCurrent(event.target.value)} required/></label><label><span>{t('password.replacement')}</span><input type="password" autoComplete="new-password" minLength={12} placeholder={t('password.minimum')} value={replacement} onChange={event=>setReplacement(event.target.value)} required/></label><label><span>{t('password.confirmation')}</span><input type="password" autoComplete="new-password" minLength={12} value={confirmation} onChange={event=>setConfirmation(event.target.value)} required/></label><button className="primary" disabled={busy||replacement.length<12}>{busy?t('password.changing'):t('password.change')}</button></section>{notice&&<p>{notice}</p>}</form>
}
  ...
  return <div className="chat-layout">
    ...
    <div className="chat-main panel">
      ...
      <div className="messages" ref={messagesRef} onScroll={event=>{const element=event.currentTarget;stickToLatest.current=element.scrollHeight-element.scrollTop-element.clientHeight<90}}>
		{entries.length === 0 && <div className="empty-chat"><div className="radar"><Activity size={35}/></div><h2>{t('chat.emptyTitle')}</h2><p>{t('chat.emptyText')}</p><div className="suggestions">
		  {['chat.suggestionDisk','chat.suggestionIncident','chat.suggestionHistory'].map((key) => <button key={key} onClick={() => setMessage(t(key))}>{t(key)}<ChevronRight size={14}/></button>)}
        </div></div>}
...
function CommandExplanationPanel({review}:{review?:CommandReview}){
  ...
  if(!review)return null
		if(review.status==='pending')return <div className="command-review-panel pending" role="status" aria-live="polite"><div className="command-review-pending"><span className="review-agent-icon"><LoaderCircle className="spin" size={17}/></span><span><b>{t('approval.explanationWorking')}</b><small>{t('approval.explanationWorkingText')}</small></span></div></div>
...
function HostsPage({ hosts, refresh }: {hosts:Host[];refresh:()=>Promise<void>}) {
  ...
  const probe = async (host:Host) => { try { const info = await api.probe(host.id); setNotice(`${host.name}: ${Object.values(info).join(' · ')}`) } catch(err){setNotice(errorText(err))} }
	...
	{showForm && <form className="host-form panel" onSubmit={save}><div className="host-form-head"><div><h3>{editing?t('hosts.editTitle'):t('hosts.createTitle')}</h3><p>{editing?t('hosts.editHelp'):t('hosts.createHelp')}</p></div><button type="button" className="close-button" title={t('common.close')} onClick={()=>setShowForm(false)}><X size={16}/></button></div><div className="form-grid host-fields">
	  ...
	  <label><span>{t('hosts.address')}</span><input value={form.address} onChange={event=>setForm({...form,address:event.target.value})} placeholder="192.0.2.10 or server.example.com" required/></label>
	  ...
	  <label><span>{t('hosts.user')}</span><input value={form.user} onChange={event=>setForm({...form,user:event.target.value})} placeholder="ops" required/></label>
	  ...
	  {form.auth_type==='password'&&<label><span>{t('hosts.sshPassword')}</span><input type="password" autoComplete="new-password" value={form.password} onChange={event=>setForm({...form,password:event.target.value})} placeholder={editing?t('hosts.keepPassword'):t('common.required')} required={!editing}/></label>}
	  {form.auth_type==='key'&&<div className="private-key-field"><span>{t('hosts.privateKey')}</span><label className={`private-key-picker ${privateKeyError||missingPrivateKey?'invalid':''}`} title={privateKeyName||t('hosts.chooseKey')}><UploadCloud size={15}/><span><b>{privateKeyName||(existingPrivateKey?t('hosts.storedKey'):t('hosts.choosePrivateKey'))}</b><small>{privateKeyName?t('hosts.readyEncrypt'):existingPrivateKey?t('hosts.encryptedConfigured'):t('hosts.keyLimit')}</small></span><input key={privateKeyInputKey} type="file" onChange={event=>void choosePrivateKey(event)}/></label>{(privateKeyError||missingPrivateKey)&&<small className="private-key-error">{privateKeyError||t('hosts.keyRequired')}</small>}</div>}
	  ...
	  <label><span>{t('hosts.proxyURL')}</span><input value={form.proxy_url} onChange={event=>setForm({...form,proxy_url:event.target.value,...(!event.target.value?{proxy_username:'',proxy_password:''}:{})})} placeholder="socks5://127.0.0.1:1080"/></label>
	  ...
	  {form.proxy_url&&form.proxy_username&&<label><span>{t('hosts.proxyPassword')}</span><input type="password" autoComplete="new-password" value={form.proxy_password} onChange={event=>setForm({...form,proxy_password:event.target.value})} placeholder={editing?t('hosts.keepPassword'):''}/></label>}
...
function ModelsPage({providers,health,refresh}:{providers:ModelProvider[];health:Health|null;refresh:()=>Promise<void>}) {
  ...
  return <div className="page-stack">
	<div className="page-actions"><div><p>{t('models.title')}</p><span>{t('models.subtitle')}</span></div><button className="primary" onClick={openCreate}><Plus size={16}/>{t('models.add')}</button></div>
    {notice&&<div className="notice">{notice}<button onClick={()=>setNotice('')}><X size={14}/></button></div>}
	<div className="model-summary panel"><div><span>{t('models.activeRoute')}</span><b>{health?.model?.name||t('models.noModel')}</b><small>{health?.model?.model||t('models.addToChat')}</small></div><div className={`model-signal ${health?.agent_available?'ready':''}`}><CircleDot size={16}/>{health?.agent_available?t('models.ready'):t('models.offline')}</div><div><span>{t('models.source')}</span><b>{health?.model?.source||'none'}</b><small>{health?.model?.provider_id||t('models.environmentFallback')}</small></div></div>
    {showForm&&<form className="model-form panel" onSubmit={save}>
	  <div className="model-form-head"><div><h3>{editing?t('models.editTitle'):t('models.newTitle')}</h3><p>{editing?t('models.editHelp'):t('models.newHelp')}</p></div><button type="button" className="close-button" title={t('common.close')} onClick={()=>setShowForm(false)}><X size={16}/></button></div>
      <div className="form-grid model-fields">
		<label><span>{t('models.displayName')}</span><input value={form.name} onChange={event=>setForm({...form,name:event.target.value})} placeholder={t('models.namePlaceholder')} required/></label>
...
function LogsPage(){
  ...
  return <div className="logs-page page-stack">
	...
	<div className="logs-meta"><span>{t('logs.entries',{count:entries.length})}</span><span>{t('logs.capture',{level:minimumLevel.toUpperCase()})}</span><span>{logFile?t('logs.file',{file:logFile}):t('logs.fileDisabled')}</span></div>
    ...
    <div className="log-stream panel">
	  ...
	  {!entries.length&&!logError&&<Empty icon={<FileText/>} title={t('logs.emptyTitle')} text={t('logs.emptyText')}/>}
    </div>
  </div>
}
