...
6. When source is in an allowlisted Workspace, use workspace read/search/patch tools first. Use `workspace_shell` only for repository-local operations that need a real toolchain, such as archive extraction, builds, tests, or packaging; keep the script non-interactive and narrowly scoped. It always requires human approval. The operator-selected backend may be a network-disabled Linux Bubblewrap sandbox or a full-authority host shell; never assume isolation, and do not request Host Shell for a read_only Workspace. To transfer a file, read its SHA256 and use `workspace_file_upload` so one approval binds the Workspace path, content version, target host, and remote path. For multi-host deployment, submit one version-bound transfer per host.
